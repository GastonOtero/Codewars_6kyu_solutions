...
codes are treated as single special characters, and usually are transmitted as 
separate words.
...
decodeMorse = function(morseCode){
  let translated = morseCode.split(' ').map(x => MORSE_CODE[x] || ' ').join('').trim();
  return translated.replace(/\s{2}/g, " ");
}
