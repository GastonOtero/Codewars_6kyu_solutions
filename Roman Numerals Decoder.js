...
function solution (roman) {
    let result = 0;
    let romanDict = {M:1000, D:500, C:100, L:50, X:10, V:5, I:1};
    let doubleLetters = ['CM', 'CD', 'XC', 'XL', 'IX', 'IV'];
    
    for (let i = 0; i < roman.length; i++){
        doubleLetters.indexOf(roman[i] + roman[i + 1]) === -1 ? result += romanDict[roman[i]] : result -= romanDict[roman[i]];
    }
    return result;
}